  ...
  <div>
    ...
    <div class="followers-list">
      <div v-for="(user, index) in followersList" :key="index" class="follower-item">
		...
		<FollowerButton :user="user" :index="index" :token="token" @toggleFollow="toggleFollow" />
	  </div>
    </div>
  </div>
</template>
...
<script>
import FollowerButton from "pages/mark/FollowerButton"; // 你的关注按钮组件路径
...
export default {
  ...
  methods: {
	...
	toggleFollow(index) {
	  this.$set(this.followersList, index, {
	    ...this.followersList[index],
